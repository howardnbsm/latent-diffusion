...
torch._six = types.SimpleNamespace(
    string_classes = six.string_types,        # 通用字串類型
    integer_types = six.integer_types,        # 通用整數類型
    container_abcs = container_abcs       # container abstract base classes
)
...
def get_parser():
    ...
    parser.add_argument(
        "--fid_real_dir",
        type=str,
        help="real images directory for FID computation",
        default=True,
    )
    return parser
